...
import sys
sys.path.insert(0, "Self-Attention")
from Self_Attention_Layer import SelfAttention
from Convolution import Dense_Spectral, Conv2D_Spectral, Conv2DTranspose_Spectral
import numpy as np
import os
...
from tensorflow.keras.layers import Conv2D, AveragePooling2D, Activation, Dropout, Conv2DTranspose, AvgPool2D
...
class GAN():
    ...
    def build_generator(self):
        ...
        input = Input((self.noise_size,))
        x = Dense(4 * 4 * 256, activation='linear')(input)
        x = Reshape((4, 4, 256))(x)

        x = Conv2DTranspose(256, kernel_size=[2, 2], strides=[2, 2], padding="same", kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.02))(x)
        #x = Conv2DTranspose_Spectral(filters=256, kernel_size=(2,2), strides=(2,2), padding="same", spectral_normalization=True, kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.02))(x)
        x = BatchNormalization(momentum=0.9, epsilon=epsilon)(x)
        x = LeakyReLU(alpha=0.2)(x)

        x = Conv2DTranspose(128, kernel_size=[2, 2], strides=[2, 2], padding="same", kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.02))(x)
        #x = Conv2DTranspose_Spectral(filters=128, kernel_size=(2, 2), strides=(2, 2), padding="same", spectral_normalization=True, kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.02))(x)
        x = BatchNormalization(momentum=0.9, epsilon=epsilon)(x)
        x = LeakyReLU(alpha=0.2)(x)
        ...
        x = SelfAttention(ch=128, name="sa1")(x)

        x = Conv2DTranspose(64, kernel_size=[2, 2], strides=[2, 2], padding="same", kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.02))(x)
        #x = Conv2DTranspose_Spectral(filters=64, kernel_size=(2, 2), strides=(2, 2), padding="same", spectral_normalization=True, kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.02))(x)
        x = BatchNormalization(momentum=0.9, epsilon=epsilon)(x)
        x = LeakyReLU(alpha=0.2)(x)

        #x = SelfAttention(ch=64, name="sa2")(x)

        x = Conv2D(3, kernel_size=[3, 3], strides=[1, 1], padding="same")(x)
        #x = Conv2D_Spectral(filters=3, kernel_size=(3,3), strides=(1,1), padding="same", spectral_normalization=True)(x)
        x = Activation("tanh")(x)
        model = Model(input, x)
        return model
    ...
    def build_discriminator(self):
        ...
        input = Input(self.img_shape)

        x = Conv2D(64, (3, 3), strides=2, padding='same')(input)
        # x = Conv2D_Spectral(filters=64, kernel_size=(3,3), strides=(2,2), padding="same", spectral_normalization=True)(input)
        x = BatchNormalization(momentum=0.8)(x)
        x = LeakyReLU(alpha=0.2)(x)
        x = Dropout(0.25)(x)

        x = Conv2D(128, (3, 3), strides=2, padding='same')(x)
        # x = Conv2D_Spectral(filters=128, kernel_size=(3, 3), strides=(2, 2), padding="same", spectral_normalization=True)(x)
        x = BatchNormalization(momentum=0.8)(x)
        x = LeakyReLU(alpha=0.2)(x)
        x = Dropout(0.25)(x)

        x = Conv2D(192, (3, 3), strides=2, padding='same')(x)
        # x = Conv2D_Spectral(filters=256, kernel_size=(3, 3), strides=(2, 2), padding="same", spectral_normalization=True)(x)
        x = BatchNormalization(momentum=0.8)(x)
        x = LeakyReLU(alpha=0.2)(x)
        x = Dropout(0.25)(x)
        ...
        x = Dropout(0.25)(x)
        # attention
        #x = SelfAttention(ch=256, name="sa1")(x)
        ...
        x = Dropout(0.25)(x)
        # attention
        #x = SelfAttention(ch=256, name="sa2")(x)

        x = Conv2D(512, (3, 3), strides=1, padding='same')(x)
        # x = Conv2D_Spectral(filters=512, kernel_size=(3, 3), strides=(1, 1), padding="same", spectral_normalization=True)(x)
        x = BatchNormalization(momentum=0.8)(x)
        x = LeakyReLU(alpha=0.2)(x)
        x = Dropout(0.25)(x)
    ...
    def train(self, epochs, batch_size=128, save_images=100, save_model=2000, interval_history = 100):
        subs = os.listdir("save_attn")
        new_subdir = os.path.join("save_attn", str(len(subs) + 1))
        ...
        for epoch in range(1, epochs+1):
            ...
            for _ in range(UNROLLED_STEPS):
                copy_idx = np.random.randint(0, X_train.shape[0], half_batch)
                copy_imgs = X_train[copy_idx]
                copy_noise = np.random.normal(0,1, (half_batch, self.noise_size))
                copy_gen_imgs = self.generator.predict(copy_noise)

                copy_true_labels = np.ones((half_batch,1))
                noise_copy_true_labels = noise_label(copy_true_labels, 0.05)
                copy_d_real_statics = self.discriminator.train_on_batch(copy_imgs, noise_copy_true_labels)

                copy_fake_labels = np.zeros((half_batch, 1))
                noise_copy_fake_labels = noise_label(copy_fake_labels, 0.05)
                copy_d_fake_statics = self.discriminator.train_on_batch(copy_gen_imgs, noise_copy_fake_labels)
            ...
            if epoch % interval_history == 0:
                ...
                self.plot_history(dict_history, new_history_dir, epoch)
    ...
    def plot_history(self, dict_, save_dir, epoch):
        # dict_history = {"D_loss_real": [], "D_loss_fake": [], "D_acc_real": [], "D_acc_fake": [], "G_loss":[]}
...
if __name__ == "__main__":
    gan = GAN()
    gan.train(epochs=30000, batch_size=256,save_images=5, save_model=10, interval_history=5)
